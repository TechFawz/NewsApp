...
const clientId =
  '10510044017-mubqkg5u5c3uu8cp1vnst04gqmi80laq.apps.googleusercontent.com';
function LoginPage() {
  ...
  const onSuccess = (res) => {
    axios
      .get(`http://${ip}:8000/check_google_login`, { params: res.profileObj })
      .then(
        (ress) => {
  ...
  useEffect(() => {
    if (Seen) {
      document.getElementById('LoginDivShowPasswordIcon').style.display =
        'none';
    ...
    <div className="ContaintContainer">
      ...
      <div className="LoginContainer">
        ...
        <div>
          ...
          <div className="CreateAccountButton" onClick={clickongoogleapi}>
            {' '}
            <FontAwesomeIcon icon={faGoogle} className="GoogleIcon" /> Login
            With Google
          </div>
          <GoogleLogin
            clientId={clientId}
...
function CheckLogin(id, password, SetWrongPassword, navigate) {
  ...
  axios.get(`http://${ip}:8000/check_login`, { params: login_details }).then(
    (res) => {
