...
function ProfileSetting() {
    ...
    const [details , SetDetails] = useState({FirstName:"",profile_url:"",MailId:""});
        ...
        <div className='ProfileSetting'>
            ...
            <div className='ProfileSettingCenter'>
                ...
                <div className='AllButton'>
                    <div className='PasswordEditButtom' onClick={()=>{navigate("/setting/edit_password")}}>Edit Password</div>
                    <div className='ProfileEditButtom' onClick={()=>{navigate("/setting/edit_profile")}}>Edit Profile</div>
                </div>
            </div>
        </div>
    )
...
function EditProfileSetting() {
    ...
    return (
        <div className='ProfileSetting'>
            ...
            <div className='ProfileSettingCenter'>
                <img src={image} className="ProfileImageSetting" onClick={()=>{InputImage.current.click()}}/>
                <input ref={InputImage} type="file" id="img" name="img" accept="image/*" style={{display:"none"}} onChange={ChangeImage}/>
                <input type="text" ref={InputName} className='UserNameInput' placeholder='Enter User Name'/>
                <input type="text" ref={InputMail} className='UserNameInput' placeholder='Enter Mail Id'/>
                <div className='AllButton'>
                    <div className='PasswordEditButtom' onClick={()=>{navigate("/setting/profile")}}>Cancel</div>
                    <div className='ProfileEditButtom' onClick={()=>{EditProfileDataBackend(InputName.current.value,InputMail.current.value,InputImage.current.files[0],navigate)}}>Save</div>
                </div>
            </div>
        </div>
    )
...
function EditPassword()
{
    ...
    return(
        ...
        <div className='ProfileSettingCenter'>
            <input ref={OP} type="password"  className='UserNameInput' placeholder='Enter Old Password'/>
            <input ref={NP} type="password"  className='UserNameInput'  placeholder='Enter New Password'/>
            <input ref={NCP} type="text"  className='UserNameInput' placeholder='Confirm New Password'/>
            <p className="WrongPasword" style={StyleForWrongPassword()}>{resone}</p>
            <div className='AllButton'>
                <div className='PasswordEditButtom' onClick={()=>{navigate("/setting/profile")}}>Cancel</div>
                <div className='ProfileEditButtom' onClick={()=>{EditPasswordBackend(OP.current.value,NP.current.value,NCP.current.value,SetWrongPassword,SetResone,navigate)}}>Save</div>
            </div>
        </div>
    </div>
    )
...
function EditPasswordBackend(OP,NP,NCP,SetWrongPassword,SetResone,navigate)
{
    ...
    axios.get('http://35.164.216.59:8000/edit_password', {params: data , headers: { "authorization": localStorage.getItem("token") } }).then(res => {
...
function EditProfileDataBackend(name,mail,image,navigate)
{
    ...
    fetch("https://api.cloudinary.com/v1_1/diu2ciwjz/image/upload",options).then(res=>res.json()).then(res=>{
     ...
     axios.get('http://35.164.216.59:8000/edit_user_data', {params: UserData , headers: { "authorization": localStorage.getItem("token") } }).then(res => {
